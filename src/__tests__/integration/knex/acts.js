...
describe("Test dynamic SQL queries built by Knex", () => {
  it("should display a correct where clause", async () => {
    ...
    it("should display a correct where clause with hospitals", async () => {
      ...
      expect(sql).toMatchInlineSnapshot(
        `"select * from \\"acts\\" where (\\"acts\\".\\"deleted_at\\" is null and acts.hospital_id in (1) and examination_date >= TO_DATE('2019-01-01', 'YYYY-MM-DD') and examination_date <= TO_DATE('2020-02-02', 'YYYY-MM-DD'))"`
      )
    })
})
